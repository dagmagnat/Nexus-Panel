...
if [[ -z "$BACKUP_ROOT" || ! -d "$BACKUP_ROOT/files" || ! -f "$BACKUP_ROOT/TARGET_DIR" ]]; then
  echo "[ERR] Укажи каталог резервной копии spectrum-clear-2.2.1-* первым аргументом." >&2
  exit 1
fi
...
MANIFEST="$BACKUP_ROOT/SPECTRUM_CLEAR_2_2_1_FILES.txt"
...
echo "[INF] data/app.db не восстанавливается автоматически: обновление её не меняет."
