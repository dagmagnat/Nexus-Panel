...
clone_or_update_repo() {
  ...
  if [ -d "$APP_DIR/.git" ]; then
    ...
    backup_tracked_source_changes
    stop_existing_aggregator_stack || return 1
...
preflight_public_ip_caddy() {
  local state_file="/opt/nexus-local-panels/${INSTANCE_NAME}/state.json"
  [ -f "$state_file" ] || return 0
  local needs_public_ip
  needs_public_ip="$(python3 - "$state_file" <<'PY'
import json, sys
with open(sys.argv[1], encoding='utf-8') as stream:
    state = json.load(stream)
print('1' if any(p.get('mode') == 'ip' and p.get('certificate') == 'public'
                 for p in state.get('providers', {}).values()) else '0')
PY
  )" || { err "Не удалось прочитать настройки локальных панелей. Остановка отменена."; return 1; }
  [ "$needs_public_ip" = "1" ] || return 0
  # Must match the IP-certificate image generated by scripts/local-panels.py.
  # Fetch before any down/stop/rm, including the source-update path.
  info "Проверяю и загружаю Caddy для IP-сертификата до остановки панели..."
  if ! docker pull caddy:2.11.4; then
    err "Не удалось загрузить caddy:2.11.4. Работающие контейнеры не остановлены этим обновлением. Проверьте доступ к Docker Hub и повторите."
    return 1
  fi
}
...
stop_existing_aggregator_stack() {
  ...
  preflight_public_ip_caddy || return 1
