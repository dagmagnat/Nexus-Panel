...
Использование:
  sudo bash apply-spectrum-clear-2.2.sh
  sudo bash apply-spectrum-clear-2.2.sh --target /opt/3xui-aggregator
  sudo bash apply-spectrum-clear-2.2.sh --no-build
...
if [[ ! -f "$TARGET_DIR/public/css/spectrum-clear.css" ]]; then
  echo "[ERR] Обновление 2.2.0 устанавливается поверх Nexus Panel Spectrum Clear 2.x." >&2
  exit 1
fi

MANIFEST="$PATCH_ROOT/SPECTRUM_CLEAR_2_2_FILES.txt"
ROLLBACK_SCRIPT="$PATCH_ROOT/rollback-spectrum-clear-2.2.sh"
[[ -f "$MANIFEST" ]] || { echo "[ERR] Нет SPECTRUM_CLEAR_2_2_FILES.txt." >&2; exit 1; }
[[ -f "$ROLLBACK_SCRIPT" ]] || { echo "[ERR] Нет rollback-spectrum-clear-2.2.sh." >&2; exit 1; }
...
BACKUP_ROOT="$BACKUP_BASE/spectrum-clear-2.2.0-$STAMP"
mkdir -p "$BACKUP_ROOT/files"
printf '%s\n' "$TARGET_DIR" > "$BACKUP_ROOT/TARGET_DIR"
cp -a "$MANIFEST" "$BACKUP_ROOT/SPECTRUM_CLEAR_2_2_FILES.txt"
cp -a "$ROLLBACK_SCRIPT" "$BACKUP_ROOT/rollback-spectrum-clear-2.2.sh"
chmod 700 "$BACKUP_ROOT/rollback-spectrum-clear-2.2.sh"
...
echo "[INF] Устанавливаю только файлы Nexus Panel 2.2.0; data/ и .env не изменяются."
while IFS= read -r relative; do
  [[ -n "$relative" ]] || continue
...
rollback_now() {
  ...
  bash "$BACKUP_ROOT/rollback-spectrum-clear-2.2.sh" "$BACKUP_ROOT" --no-build || true
}
...
if [[ "$NO_BUILD" -eq 1 ]]; then
  ...
  echo "[INF] Откат: bash '$BACKUP_ROOT/rollback-spectrum-clear-2.2.sh' '$BACKUP_ROOT'"
  exit 0
fi
...
docker compose ps
echo "[OK] Nexus Panel 2.2.0 установлен. Клиенты, UUID и ссылки сохранены."
echo "[INF] Резервная копия: $BACKUP_ROOT"
echo "[INF] Откат: bash '$BACKUP_ROOT/rollback-spectrum-clear-2.2.sh' '$BACKUP_ROOT'"
